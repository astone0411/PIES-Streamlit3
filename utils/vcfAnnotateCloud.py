...
# ============================================================
def process_batch_to_dict(batch: List[Tuple[str,int,str,str]], genome_build: str = "GRCh38") -> List[dict]:
    region_inputs = to_vep_region_strings(batch)
    res = vep_post(region_inputs, assembly=genome_build)
...
# ============================================================
def annotate_vcf_to_json(in_vcf: str, out_json: str, batch_size: int = 200, genome_build: str = "GRCh38"):
    all_records = []
    ...
    for var in parse_vcf(in_vcf):
        ...
        if len(batch) >= batch_size:
            all_records.extend(process_batch_to_dict(batch, genome_build=genome_build))
            batch.clear()
